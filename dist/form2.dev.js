...
function Validator(options) {
  ...
  function validate(inputElement, rule) {
    ...
    var errorMessage; // lấy ra các rules của selecyor

    var rules = selectorRules[rule.selector]; // lặp qua từng rules & kiểm tra
  ...
  if (formElement) {
    ...
    formElement.onsubmit = function (e) {
      e.preventDefault();
      var isFormValid = true; // thực hiện lặp qua từng rule và validate
...
Validator.minLength = function (selector, min) {
  return {
    selector: selector,
    test: function test(value) {
      return value.length >= min ? undefined : "Vui l\xF2ng nh\u1EADp t\u1ED1i thi\u1EC3u ".concat(min, " k\xED t\u1EF1");
    }
  };
};
